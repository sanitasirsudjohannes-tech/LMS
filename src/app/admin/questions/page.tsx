...
import { downloadQuestionImportTemplate, readQuestionImportFile, validateQuestionRows } from '@/lib/questionImport';
...
export default function QuestionsAdminPage() {
  ...
  const handleTrainingChange = async (trId: string) => {
    ...
    setLoadingQuestions(true);
    setOperationError('');
    try { setQuestions(await StorageAPI.loadQuestionsForAdmin(trId)); }
    catch (error) { setQuestions([]); setOperationError(error instanceof Error ? error.message : 'Soal gagal dimuat.'); }
    finally { setLoadingQuestions(false); }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await Swal.fire({ icon: 'success', title: 'Soal Tersimpan', text: editingId ? 'Perubahan soal berhasil disimpan.' : 'Soal baru berhasil ditambahkan.', timer: 1800, showConfirmButton: false });
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Soal gagal disimpan.'; setOperationError(message); await Swal.fire('Gagal Menyimpan Soal', message, 'error');
    } finally { setSaving(false); }
  };

  const handleDelete = async (id: string) => {
    const result = await Swal.fire({ icon: 'warning', title: 'Hapus Soal?', text: 'Soal yang dihapus tidak dapat dipulihkan dari halaman ini.', showCancelButton: true, confirmButtonText: 'Ya, Hapus', cancelButtonText: 'Batal', confirmButtonColor: '#dc2626' });
    if (!result.isConfirmed) return;
    setOperationError('');
    try { await StorageAPI.deleteQuestion(id); reloadQuestions(); await Swal.fire({ icon: 'success', title: 'Soal Terhapus', timer: 1500, showConfirmButton: false }); }
    catch (error) { const message = error instanceof Error ? error.message : 'Soal gagal dihapus.'; setOperationError(message); await Swal.fire('Gagal Menghapus Soal', message, 'error'); }
  };

  const handleImportQuestions = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0]; event.target.value = '';
    if (!file) return;
    if (!selectedTrainingId) { await Swal.fire('Pilih Pelatihan', 'Pilih pelatihan tujuan sebelum mengimpor soal.', 'warning'); return; }
    setIsImporting(true);
    try {
      ...
      if (result.errors.length > 0) {
        ...
        await Swal.fire({ icon: 'error', title: 'File Belum Valid', html: `<p class="text-sm mb-2">Perbaiki data berikut lalu impor kembali:</p><ul class="text-left text-xs space-y-1">${details}</ul>${result.errors.length > 10 ? `<p class="text-xs mt-2">Dan ${result.errors.length - 10} kesalahan lainnya.</p>` : ''}`, confirmButtonText: 'Mengerti' });
        return;
      }
      if (result.validRows.length === 0) { await Swal.fire('Tidak Ada Soal Baru', 'Semua soal dalam file sudah ada atau file tidak berisi data.', 'info'); return; }
      const confirmation = await Swal.fire({ icon: 'question', title: 'Impor Soal?', html: `<p><b>${result.validRows.length}</b> soal valid akan ditambahkan ke <b>${selectedTrainingObj?.title || 'pelatihan terpilih'}</b>.</p>${result.duplicateCount > 0 ? `<p class="text-xs mt-2">${result.duplicateCount} soal duplikat akan dilewati.</p>` : ''}`, showCancelButton: true, confirmButtonText: 'Ya, Impor', cancelButtonText: 'Batal' });
      if (!confirmation.isConfirmed) return;
      await StorageAPI.saveQuestionsBulk(result.validRows, selectedTrainingId); reloadQuestions(selectedTrainingId);
    ...
    } catch (error) { await Swal.fire({ icon: 'error', title: 'Impor Gagal', text: error instanceof Error ? error.message : 'File tidak dapat diproses.' }); }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-t border-slate-100 dark:border-slate-800 pt-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button onClick={handleOpenCreate} disabled={!selectedTrainingId} className="px-4 py-2 bg-[#07375c] hover:bg-[#052c4a] text-white rounded-xl text-xs font-bold shadow-sm inline-flex items-center gap-1.5 disabled:opacity-50"><Plus className="w-4 h-4" /><span>Tambah Soal</span></button>
          </div>
        </div>
      </div>
      ...
      {isModalOpen && <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 max-w-xl w-full space-y-4 shadow-xl max-h-[90vh] overflow-y-auto"><div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3"><h3 className="text-base font-bold text-slate-900 dark:text-white">{editingId ? 'Edit Soal' : `Tambah Soal (${selectedTrainingObj?.title})`}</h3><button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-slate-600"><X className="w-5 h-5" /></button></div><form onSubmit={handleSave} className="space-y-4">
        ...
        <label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={active} onChange={(e) => setActive(e.target.checked)} />Soal aktif</label>
