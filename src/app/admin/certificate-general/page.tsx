    ...
    <form onSubmit={save} className="space-y-5">
      <section className="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
        ...
        <div className="grid gap-5 border-t border-slate-100 pt-5 dark:border-slate-800 sm:grid-cols-2">
          {[{ kind: 'signature' as const, label: 'Tanda Tangan Direktur', preview: signaturePreview }, { kind: 'stamp' as const, label: 'Cap Rumah Sakit', preview: stampPreview }].map(x => <div key={x.kind} className="space-y-3"><p className="text-xs font-semibold">{x.label} (PNG)</p><div className="relative flex h-32 items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-800">{x.preview ? <Image src={x.preview} alt={x.label} fill sizes="320px" unoptimized className="object-contain p-2" /> : <span className="text-xs text-slate-400">Belum ada gambar</span>}</div><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-bold dark:border-slate-700"><Upload className="h-4 w-4" />Pilih PNG<input type="file" accept="image/png" className="hidden" onChange={e => void chooseFile(x.kind, e.target.files?.[0])} /></label></div>)}
        </div>
      </section>
