  ...
  return <div className="space-y-6">
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800 p-1.5 rounded-xl border border-slate-200 dark:border-slate-700 w-full md:w-auto">
        ...
        <select aria-label="Pelatihan" value={selectedTrainingId} onChange={event => handleTrainingChange(event.target.value)} className="w-full md:w-[260px] bg-transparent text-xs font-bold text-slate-900 dark:text-white focus:outline-none pr-2 py-1.5 truncate">
    ...
    <div className="md:hidden space-y-3">
      {loading ? <div className="rounded-2xl border border-slate-200 bg-white p-6 text-center dark:border-slate-800 dark:bg-slate-900"><LontarLoadingSpinner size="md" text="Memuat data..." /></div> : rows.length ? rows.map(row => (
        <div key={row.user_id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="grid grid-cols-3 gap-2 mt-4 text-center"><div className="rounded-xl bg-slate-50 dark:bg-slate-800 p-2"><span className="block text-xs text-slate-400">Pre-Test</span><strong className="text-sm font-mono">{row.pre_score ?? '-'}</strong></div><div className="rounded-xl bg-slate-50 dark:bg-slate-800 p-2"><span className="block text-xs text-slate-400">Post-Test</span><strong className="text-sm font-mono">{row.post_score ?? '-'}</strong></div><div className="rounded-xl bg-slate-50 dark:bg-slate-800 p-2"><span className="block text-xs text-slate-400">Sertifikat</span><strong className="block truncate text-xs font-mono">{row.certificate_number || '-'}</strong></div></div>
        </div>
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-3 sm:p-4 dark:border-slate-800 dark:bg-slate-900 flex items-center justify-between text-xs text-slate-500"><span>{rows.length} dari {totalCount}</span><div className="flex items-center gap-2"><button aria-label="Halaman sebelumnya" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1 || loading} className="lontar-icon-button rounded-lg border disabled:opacity-40"><ChevronLeft className="w-4 h-4" /></button><span className="font-semibold text-slate-900 dark:text-white">{currentPage}/{totalPages}</span><button aria-label="Halaman berikutnya" onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages || loading} className="lontar-icon-button rounded-lg border disabled:opacity-40"><ChevronRight className="w-4 h-4" /></button></div></div>
  </div>;
}
