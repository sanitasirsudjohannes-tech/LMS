...
export default function TrainingSettingsAdminPage() {
  ...
  const handleOpenEdit = (t: Training) => {
    ...
    setMaxAttempts(t.max_posttest_attempts);
    setActive(t.active);
    setIsModalOpen(true);
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    StorageAPI.saveTraining({
      ...
      max_posttest_attempts: Number(maxAttempts),
      active
    });
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 sm:p-7 max-w-lg w-full space-y-4 shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Maks. Percobaan Post-Test</label>
                  <input
                    type="number"
                    min={1}
                    max={10}
