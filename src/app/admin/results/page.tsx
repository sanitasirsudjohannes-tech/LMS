...
export default function ResultsAdminPage() {
  ...
  useEffect(() => {
    ...
    const loadPage = async () => {
      ...
      if (error) {
        ...
        const testResults = result.filter(row => row.pre_score != null || row.post_score != null);
        setRows(testResults);
  ...
  return <div className="space-y-6">
    <div className="bg-blue-50 dark:bg-blue-950/30 border-2 border-blue-300 dark:border-blue-800 rounded-2xl p-5 shadow-sm space-y-3">
      <div><h2 className="text-sm font-bold text-blue-950 dark:text-blue-100">Pilih Pelatihan</h2><p className="text-xs text-blue-700 dark:text-blue-300 mt-0.5">Setiap peserta ditampilkan satu kali dengan nilai terbaiknya.</p></div>
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
      <div className="relative w-full md:w-80"><Search className="w-4 h-4 text-slate-400 absolute left-3 top-3" /><input value={search} onChange={event => { setSearch(event.target.value); setCurrentPage(1); }} placeholder="Cari nama peserta..." className="w-full pl-9 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white" /></div>
    </div>

    {loadError && <div className="rounded-xl border border-red-300 bg-red-50 p-4 text-xs text-red-800"><strong>Hasil tes gagal dimuat.</strong> {loadError}</div>}

    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
      <div className="overflow-x-auto"><table className="w-full text-left text-xs"><thead className="bg-slate-50 dark:bg-slate-800/80 text-slate-500 font-semibold border-b border-slate-200 dark:border-slate-800 uppercase tracking-wider"><tr><th className="p-4">Peserta</th><th className="p-4 text-center">Pre-Test</th><th className="p-4 text-center">Post-Test Terbaik</th><th className="p-4 text-center">Status</th><th className="p-4">Nomor Sertifikat</th></tr></thead>
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">{loading ? <tr><td colSpan={5} className="p-8 text-center text-slate-400">Memuat data...</td></tr> : rows.length ? rows.map(row => <tr key={row.user_id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40"><td className="p-4"><p className="font-bold text-slate-900 dark:text-white">{row.full_name}</p><p className="text-slate-400 text-[11px]">{row.email} • {row.institution}</p></td><td className="p-4 text-center font-mono font-bold">{row.pre_score ?? '-'}</td><td className="p-4 text-center font-mono text-base font-bold">{row.post_score ?? '-'}</td><td className="p-4 text-center">{row.status === 'Lulus' ? <span className="inline-flex items-center gap-1 text-emerald-700 font-bold"><CheckCircle2 className="w-3 h-3" />Lulus</span> : row.status === 'Belum Lulus' ? <span className="inline-flex items-center gap-1 text-red-700 font-bold"><XCircle className="w-3 h-3" />Belum Lulus</span> : <span className="inline-flex items-center gap-1 text-slate-500"><Clock className="w-3 h-3" />{row.status}</span>}</td><td className="p-4 font-mono">{row.certificate_number || '-'}</td></tr>) : <tr><td colSpan={5} className="p-8 text-center text-slate-400">Belum ada peserta yang mengerjakan tes.</td></tr>}</tbody></table></div>
