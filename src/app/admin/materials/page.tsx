      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white dark:bg-slate-100 dark:text-slate-900 rounded-xl text-xs font-bold transition-all shadow-sm flex items-center gap-1.5"
          >
      ...
      <div className="space-y-3">
        {materials.length > 0 ? (
          materials.map((m) => (
            ...
            >
              <div className="flex items-start gap-3">
                <span className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white font-mono text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">
                  #{m.order_number}
                </span>
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`px-2 py-0.5 rounded text-[10px] font-semibold ${
                      m.active ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/80 dark:text-emerald-300' : 'bg-slate-100 text-slate-500'
                    }`}>
                  ...
                  <div className="flex items-center gap-3 text-[11px] text-slate-400 mt-2 font-mono">
                    ...
                    {m.content_url && (() => {
                      ...
                      return type === 'pdf' ? (
                        <span className="flex items-center gap-1 text-red-600 dark:text-red-400 font-bold bg-red-50 dark:bg-red-950/40 px-2 py-0.5 rounded">
                          <FileText className="w-3 h-3" /> Media: PDF
                        </span>
                      ) : (
                        <span className="flex items-center gap-1 text-amber-600 dark:text-amber-400 font-bold bg-amber-50 dark:bg-amber-950/40 px-2 py-0.5 rounded">
                          <Video className="w-3 h-3" /> Media: VIDEO
                        </span>
                      );
                    })()}
                  </div>
              ...
              <div className="flex items-center gap-2 self-end sm:self-center">
                <button
                  ...
                  className="px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100"
                >
                ...
                <button
                  ...
                  className="p-2 text-slate-400 hover:text-red-600 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"
                >
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 text-center text-slate-400 text-xs">
            Belum ada materi untuk pelatihan &quot;{selectedTrainingObj?.title}&quot;. Klik tombol Tambah Materi di atas untuk menambahkan.
          </div>
        )}
      </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 max-w-xl w-full space-y-4 shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <h3 className="text-base font-bold text-slate-900 dark:text-white">
                {editingId ? 'Edit Materi' : `Tambah Materi (${selectedTrainingObj?.title})`}
              </h3>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Contoh: Materi 1: Orientasi Keselamatan"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Deskripsi Ringkas</label>
                <input
                  type="text"
                  value={description}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Konten Bacaan (Teks/HTML)</label>
                <textarea
                  rows={6}
                  required
                  value={content}
                  ...
                  className="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white font-sans"
                />
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">URL Media / PDF / Video Embed (Opsional)</label>
                <input
                  type="url"
                  value={contentUrl}
                  onChange={(e) => setContentUrl(e.target.value)}
                  placeholder="Link file PDF (misal: https://.../materi.pdf) atau URL Embed Video"
                  className="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Durasi Minimum (Detik)</label>
                  <input
                    type="number"
                    min={0}
                    required
