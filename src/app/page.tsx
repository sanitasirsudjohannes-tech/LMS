  ...
  return (
    <div className="space-y-16 py-4">
      <section className="text-center space-y-6 max-w-3xl mx-auto pt-6">
        ...
        <h1 className="text-3xl sm:text-5xl font-bold tracking-tight text-slate-900 dark:text-white leading-tight">LONTAR</h1>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 pt-2 min-h-[48px]">
          {authLoading ? (
            ...
            <>
              ...
              <Link href="/register" className="inline-flex items-center gap-2 px-6 py-3 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-900 dark:text-white font-medium rounded-xl text-sm transition-all">
      ...
      <section className="bg-white dark:bg-slate-900 rounded-2xl p-6 sm:p-10 border border-slate-200 dark:border-slate-800 shadow-sm space-y-8">
        <div className="text-center max-w-xl mx-auto space-y-2">
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Setiap tahap harus diselesaikan secara tertib. Sistem secara otomatis mengunci tahapan lanjutan.</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-6 gap-3 text-center">
          {[
            { step: '1', title: 'Pendaftaran', desc: 'Buat akun peserta', icon: Users },
            { step: '2', title: 'Login', desc: 'Akses sistem', icon: Lock },
            { step: '3', title: 'Pre-Test', desc: 'Ukur awal kemampuan', icon: FileCheck2 },
            { step: '4', title: 'Materi 1..N', desc: 'Timer minimum', icon: BookOpen },
            { step: '5', title: 'Post-Test', desc: 'Evaluasi akhir', icon: GraduationCap },
            { step: '6', title: 'Sertifikat', desc: 'Unduh PDF & QR', icon: Award }
          ].map((item) => (
            <div key={item.step} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 flex flex-col items-center justify-between text-slate-800 dark:text-slate-200">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Langkah {item.step}</span>
              <div className="w-9 h-9 rounded-lg bg-white dark:bg-slate-700 shadow-sm flex items-center justify-center text-slate-900 dark:text-white my-2"><item.icon className="w-4 h-4" /></div>
