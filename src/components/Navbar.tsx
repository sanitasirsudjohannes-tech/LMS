...
export default function Navbar({ children }: { children: React.ReactNode }) {
  ...
  const navItems = useMemo<NavItem[]>(() => {
    ...
    return [
      ...
      { href: '/trainings', label: 'Pelatihan Saya', icon: BookOpen },
  ...
  if (isLoggingOut) {
    return <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 dark:bg-slate-950"><LontarLoadingSpinner size="lg" text="Keluar dari akun..." /></div>;
  }

  if (authLoading) return <div className="min-h-screen">{children}</div>;
        ...
        <header className="fixed inset-x-0 top-0 z-40 h-16 border-b border-slate-200 bg-white/95 backdrop-blur dark:border-slate-800 dark:bg-slate-950/95">
          <div className="mx-auto flex h-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center gap-2">
              {pathname !== '/login' && <Link href="/login" className="rounded-lg px-3 py-2 text-xs font-semibold text-slate-700 transition-colors hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-900 sm:text-sm">Masuk</Link>}
              {pathname !== '/register' && <Link href="/register" className="rounded-lg bg-[#07375c] px-3.5 py-2 text-xs font-semibold text-white transition-colors hover:bg-[#052c4a] sm:text-sm">Daftar Akun</Link>}
            </div>
          </div>
        </header>
  ...
  const SidebarContent = ({ mobile = false }: { mobile?: boolean }) => (
    <div className="flex h-full flex-col bg-slate-50 dark:bg-slate-950">
      ...
      <div className="border-t border-slate-200 p-3 dark:border-slate-800">
        <button type="button" onClick={() => void handleLogout()} disabled={isLoggingOut} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-red-600 transition-colors hover:bg-red-50 disabled:cursor-wait disabled:opacity-60 dark:text-red-400 dark:hover:bg-red-950/30"><LogOut className="h-4 w-4" /><span>Keluar</span></button>
      </div>
    </div>
  );
