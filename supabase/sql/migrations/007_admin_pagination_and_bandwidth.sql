...
AS $$
  WITH participant_users AS (
    ...
    SELECT COUNT(*) AS total FROM public.materials WHERE training_id = p_training_id AND active
    ...
    WHERE m.training_id = p_training_id AND m.active AND mp.completed_at IS NOT NULL
    GROUP BY mp.user_id
  ), rows AS (
    SELECT
      x.user_id,
      ...
      CASE
        ...
        WHEN a.pre_score IS NOT NULL AND (mt.total = 0 OR COALESCE(mc.completed, 0) >= mt.total) THEN 'Selesai Materi'
...
AS $$
  WITH tr AS (
    ...
    SELECT DISTINCT a.user_id FROM public.test_attempts a JOIN participant_users pu ON pu.user_id = a.user_id
    WHERE a.training_id = p_training_id AND a.test_type = 'pretest'
  ), post_users AS (
    SELECT DISTINCT a.user_id FROM public.test_attempts a JOIN participant_users pu ON pu.user_id = a.user_id
    WHERE a.training_id = p_training_id AND a.test_type = 'posttest'
  ), passed_users AS (
    SELECT DISTINCT a.user_id FROM public.test_attempts a JOIN participant_users pu ON pu.user_id = a.user_id CROSS JOIN tr
    WHERE a.training_id = p_training_id AND a.test_type = 'posttest' AND a.score >= tr.passing_score
  ), completed_users AS (
    SELECT pu.user_id FROM pre_users pu
    WHERE NOT EXISTS (
      SELECT 1 FROM public.materials m
      WHERE m.training_id = p_training_id AND m.active
        AND NOT EXISTS (
          SELECT 1 FROM public.material_progress mp
          WHERE mp.user_id = pu.user_id AND mp.material_id = m.id AND mp.completed_at IS NOT NULL
        )
    )
  )
  SELECT jsonb_build_object(
    ...
    'certificatesIssued', (
      SELECT COUNT(*) FROM public.certificates c JOIN participant_users pu ON pu.user_id = c.user_id
